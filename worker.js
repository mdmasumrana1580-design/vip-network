...
function defaultState(){return{channels:[],categories:['ALL','SPORTS','BD','INDIA','OTHER','MOVIE & SERIES'],movieChannels:[],notice:{text:'',type:'Information',enabled:true},headline:''};}
async function readState(e){try{const r=await kv(e).get(STATE_KEY);if(!r)return defaultState();const s=JSON.parse(r);return{...defaultState(),...s,channels:Array.isArray(s.channels)?s.channels.map(norm).filter(x=>x.category!=='MOVIE & SERIES'):[],movieChannels:Array.isArray(s.movieChannels)?s.movieChannels.map(x=>({...norm(x),category:'MOVIE & SERIES'})):[],categories:Array.isArray(s.categories)&&s.categories.length?[...new Set(s.categories.filter(x=>String(x).toUpperCase()!=='NEW STYLE').map(x=>String(x).toUpperCase()))]:defaultState().categories}}catch{return defaultState()}}
async function saveState(e,s){const state={...defaultState(),...s,channels:Array.isArray(s.channels)?s.channels.map(norm):[],categories:Array.isArray(s.categories)?s.categories.filter(x=>String(x).toUpperCase()!=='NEW STYLE'):defaultState().categories,movieChannels:Array.isArray(s.movieChannels)?s.movieChannels.map(x=>({...norm(x),category:'MOVIE & SERIES'})):[]};await kv(e).put(STATE_KEY,JSON.stringify(state));return state;}
...
async function handle(r,e){
  ...
  if(r.method==='GET'&&p==='/api/state'){const s=await readState(e);return withCors(json({channels:s.channels||[],movieChannels:s.movieChannels||[],categories:s.categories||[],notice:s.notice||{},headline:s.headline||''}))}
  if(r.method==='GET'&&p==='/api/playlist'){const s=await readState(e);const kind=(u.searchParams.get('type')||'tv').toLowerCase();return withCors(json({channels:kind==='movies'?(s.movieChannels||[]):(s.channels||[]),categories:s.categories||[]}))}
  ...
  if(p==='/api/admin/state'&&r.method==='PUT'){const b=await r.json().catch(()=>({})),old=await readState(e),s={...old,...b};if(Array.isArray(b.channels))s.channels=b.channels.map(norm).filter(x=>x.category!=='MOVIE & SERIES');if(Array.isArray(b.movieChannels))s.movieChannels=b.movieChannels.map(x=>({...norm(x),category:'MOVIE & SERIES'}));if(Array.isArray(b.categories))s.categories=b.categories.map(x=>String(x).trim()).filter(Boolean);else s.categories=old.categories;const saved=await saveState(e,s);return withCors(json({ok:true,state:saved}))}
  ...
  if(p==='/api/admin/import-m3u-url'&&r.method==='POST'){const b=await r.json().catch(()=>({}));if(!/^https?:\/\//i.test(String(b.url||'')))return withCors(json({ok:false,error:'Invalid M3U URL'},400));const x=await fetch(b.url);if(!x.ok)return withCors(json({ok:false,error:'M3U URL failed: '+x.status},400));const s=await readState(e);s.channels=parseM3U(await x.text()).filter(x=>x.category!=='MOVIE & SERIES');s.categories=defaultState().categories;return withCors(json({ok:true,count:s.channels.length,state:await saveState(e,s)}))}
  if(p==='/api/admin/import-m3u'&&r.method==='POST'){const s=await readState(e);s.channels=parseM3U(await r.text()).filter(x=>x.category!=='MOVIE & SERIES');s.categories=defaultState().categories;return withCors(json({ok:true,count:s.channels.length,state:await saveState(e,s)}))}
  if(p==='/api/admin/movie-playlist'&&r.method==='GET'){const s=await readState(e);return withCors(json({ok:true,movieChannels:s.movieChannels||[]}))}
  if(p==='/api/admin/movie-playlist'&&r.method==='PUT'){const s=await readState(e),b=await r.json().catch(()=>({}));s.movieChannels=Array.isArray(b.movieChannels)?b.movieChannels.map(x=>({...norm(x),category:'MOVIE & SERIES'})):[];const saved=await saveState(e,s);return withCors(json({ok:true,count:saved.movieChannels.length,movieChannels:saved.movieChannels}))}
  if(p==='/api/admin/movie-playlist/url'&&r.method==='POST'){const b=await r.json().catch(()=>({}));if(!/^https?:\/\//i.test(String(b.url||'')))return withCors(json({ok:false,error:'Invalid M3U URL'},400));const x=await fetch(b.url);if(!x.ok)return withCors(json({ok:false,error:'Movie M3U URL failed: '+x.status},400));const list=parseM3U(await x.text()).map(x=>({...x,category:'MOVIE & SERIES'}));const s=await readState(e);s.movieChannels=list;const saved=await saveState(e,s);return withCors(json({ok:true,count:list.length,movieChannels:saved.movieChannels}))}
  if(p==='/api/admin/movie-playlist/import'&&r.method==='POST'){const list=parseM3U(await r.text()).map(x=>({...x,category:'MOVIE & SERIES'}));const s=await readState(e);s.movieChannels=list;const saved=await saveState(e,s);return withCors(json({ok:true,count:list.length,movieChannels:saved.movieChannels}))}
  if(p==='/api/xtream/import'&&r.method==='POST'){const b=await r.json().catch(()=>({}));try{const list=await fetchXtream(b.server,b.username,b.password,b.limit||'all'),s=await readState(e);s.channels=list.filter(x=>x.category!=='MOVIE & SERIES');s.categories=defaultState().categories;const saved=await saveState(e,s);return withCors(json({ok:true,count:list.length,state:saved}))}catch(err){return withCors(json({ok:false,error:err.message||'Xtream import failed'},400))}}
