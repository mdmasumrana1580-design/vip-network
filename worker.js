...
const MOVIE_PLAYLIST_KEY='vip_movie_playlist_v1';
...
async function readMovies(e){try{return JSON.parse((await kv(e).get(MOVIE_PLAYLIST_KEY))||'[]')}catch{return[]}}
async function saveMovies(e,x){await kv(e).put(MOVIE_PLAYLIST_KEY,JSON.stringify(Array.isArray(x)?x.map(norm):[]))}
...
async function handle(r,e){
  ...
  if(p==='/api/user/login'&&r.method==='POST'){
    ...
    let created=false;
    if(!user){const salt=crypto.randomUUID();user={id:crypto.randomUUID(),username,number,numberHash:await hash(salt+number),numberSalt:salt,createdAt:new Date().toISOString(),lastLoginAt:null,lastDeviceId:null,lastDeviceName:'',activeSessionToken:null,activeDeviceId:null};users.push(user);created=true;}
  ...
  if(r.method==='GET'&&p==='/api/playlist'){const s=await readState(e);return withCors(json({channels:(s.channels||[]).filter(c=>c.category!=='MOVIE & SERIES'),categories:['ALL','SPORTS','BD','INDIA','OTHER','MOVIE & SERIES']}))}
  if(r.method==='GET'&&p==='/api/movie-playlist'){return withCors(json({channels:await readMovies(e),category:'MOVIE & SERIES'}))}
  ...
  if(p==='/api/admin/state'&&r.method==='PUT'){const b=await r.json().catch(()=>({})),old=await readState(e),s={...old,...b};if(Array.isArray(b.channels))s.channels=b.channels.map(norm).filter(c=>c.category!=='MOVIE & SERIES');if(Array.isArray(b.categories))s.categories=b.categories.map(x=>String(x).trim()).filter(Boolean).filter(x=>x!=='NEW STYLE');else s.categories=old.categories;const saved=await saveState(e,s);return withCors(json({ok:true,state:saved}))}
  if(p==='/api/admin/users'&&r.method==='GET'){const users=await readUsers(e),ds=await readDevices(e);const safe=users.map(u=>({id:u.id,username:u.username,number:u.number||'',numberSet:!!u.numberHash,createdAt:u.createdAt||null,lastLoginAt:u.lastLoginAt||null,lastDeviceId:u.lastDeviceId||null,lastDeviceName:u.lastDeviceName||'',devices:ds.filter(d=>d.username===u.username).map(d=>({deviceId:d.deviceId,name:d.name,status:d.status||'',blocked:!!d.blocked,lastSeen:d.lastSeen||null}))}));const latest=users.filter(u=>u.lastLoginAt).sort((a,b)=>new Date(b.lastLoginAt)-new Date(a.lastLoginAt))[0]||null;return withCors(json({ok:true,users:safe,latest:latest?{username:latest.username,number:latest.number||'',lastLoginAt:latest.lastLoginAt||null,lastDeviceName:latest.lastDeviceName||''}:null}))}
  if(p==='/api/admin/users/set-number'&&r.method==='POST'){const b=await r.json().catch(()=>({})),id=String(b.userId||''),number=String(b.number||'').replace(/\s+/g,'');if(!/^\+?[0-9]{6,20}$/.test(number))return withCors(json({ok:false,error:'Invalid number'},400));const users=await readUsers(e),u0=users.find(x=>x.id===id);if(!u0)return withCors(json({ok:false,error:'User not found'},404));const salt=crypto.randomUUID();u0.number=number;u0.numberSalt=salt;u0.numberHash=await hash(salt+number);u0.passwordHash=undefined;u0.salt=undefined;u0.numberUpdatedAt=new Date().toISOString();await saveUsers(e,users);return withCors(json({ok:true}))}
  ...
  if(p==='/api/admin/movie/import-m3u-url'&&r.method==='POST'){const b=await r.json().catch(()=>({}));if(!/^https?:\/\//i.test(String(b.url||'')))return withCors(json({ok:false,error:'Invalid M3U URL'},400));const x=await fetch(b.url);if(!x.ok)return withCors(json({ok:false,error:'M3U URL failed: '+x.status},400));const list=parseM3U(await x.text()).map(c=>({...c,category:'MOVIE & SERIES'}));await saveMovies(e,list);return withCors(json({ok:true,count:list.length,movies:list}))}
   if(p==='/api/admin/movie/import-m3u'&&r.method==='POST'){const list=parseM3U(await r.text()).map(c=>({...c,category:'MOVIE & SERIES'}));await saveMovies(e,list);return withCors(json({ok:true,count:list.length,movies:list}))}
   if(p==='/api/admin/movie/clear'&&r.method==='POST'){await saveMovies(e,[]);return withCors(json({ok:true,count:0}))}
   if(p==='/api/admin/movie'&&r.method==='GET')return withCors(json({ok:true,movies:await readMovies(e)}));
