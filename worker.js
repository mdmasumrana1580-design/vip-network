...
function normalizeCategory(name='',group=''){
  ...
  if(/new\s*style|newstyle/.test(t))return 'OTHER';
...
async function readMovies(e){try{const a=JSON.parse((await kv(e).get(MOVIE_KEY))||'[]');return Array.isArray(a)?a.map(norm).filter(x=>x.category==='MOVIE & SERIES'&&x.url):[]}catch{return[]}}
async function saveMovies(e,list){const a=Array.isArray(list)?list.map(norm).filter(x=>x.category==='MOVIE & SERIES'&&x.url):[];await kv(e).put(MOVIE_KEY,JSON.stringify(a));return a}
...
async function handle(r,e){
  ...
  if(p==='/api/movie-playlist'&&r.method==='GET')return withCors(json({channels:await readMovies(e),categories:['MOVIE & SERIES']}));
  if(p==='/api/admin/movie-playlist'&&r.method==='GET'){const a=await requireAdmin(r,e);if(a)return withCors(a);return withCors(json({ok:true,channels:await readMovies(e)}))}
  if(p==='/api/admin/movie/import-m3u-url'&&r.method==='POST'){const a=await requireAdmin(r,e);if(a)return withCors(a);const b=await r.json().catch(()=>({}));if(!/^https?:\/\//i.test(String(b.url||'')))return withCors(json({ok:false,error:'Invalid M3U URL'},400));const x=await fetch(b.url);if(!x.ok)return withCors(json({ok:false,error:'Movie M3U URL failed: '+x.status},400));const saved=await saveMovies(e,parseM3U(await x.text()));return withCors(json({ok:true,count:saved.length,channels:saved}))}
  if(p==='/api/admin/movie/import-m3u'&&r.method==='POST'){const a=await requireAdmin(r,e);if(a)return withCors(a);const saved=await saveMovies(e,parseM3U(await r.text()));return withCors(json({ok:true,count:saved.length,channels:saved}))}
  if(p==='/api/admin/movie/clear'&&r.method==='POST'){const a=await requireAdmin(r,e);if(a)return withCors(a);await saveMovies(e,[]);return withCors(json({ok:true,count:0}))}
  if(p==='/api/admin/state'&&r.method==='GET')return withCors(json({ok:true,state:await readState(e)}));
  if(p==='/api/admin/state'&&r.method==='PUT'){const b=await r.json().catch(()=>({})),old=await readState(e),s={...old,...b};if(Array.isArray(b.channels))s.channels=b.channels.map(norm).filter(x=>x.category!=='MOVIE & SERIES');if(Array.isArray(b.categories))s.categories=b.categories.map(x=>String(x).trim()).filter(Boolean);else s.categories=old.categories;const saved=await saveState(e,s);return withCors(json({ok:true,state:saved}))}
