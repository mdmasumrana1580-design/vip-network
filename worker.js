  ...
  for(const raw of lines){
    ...
    if(line.startsWith('#EXTINF')){
      const comma=line.indexOf(','),
            name=comma>=0?line.slice(comma+1).trim():'Live Channel',
            gm=line.match(/group-title="([^"]*)"/i),
            lm=line.match(/tvg-logo="([^"]*)"/i);

      meta={
        name:name||'Live Channel',
        category:gm?gm[1]:'Other',
        logo:lm?lm[1]:''
      };
      continue;
    }
...
async function readState(env){
  const raw=await kv(env).get(STATE_KEY);
...
async function saveState(env,state){
  await kv(env).put(STATE_KEY,JSON.stringify(state));
}

async function readDevices(env){
  const raw=await kv(env).get(DEVICES_KEY);
  try{
...
async function saveDevices(env,list){
  await kv(env).put(DEVICES_KEY,JSON.stringify(list));
}
...
async function authorized(request,env){
  ...
  const c=cookies(request),
        token=c.VIP_ADMIN_SESSION;

  if(token){
    const ok=await kv(env).get(SESSION_PREFIX+token);
...
async function handleApi(request,env){
  ...
  /*
   * ADMIN LOGIN
   * FIXED: Set-Cookie is now sent as an actual response header.
   */
  if(path==='/api/admin/login'&&request.method==='POST'){
    const b=await request.json().catch(()=>({})),
          password=String(b.password||'');

    if(!adminPassword(env)||password!==adminPassword(env)){
      return withCors(
        json(
          {
            ok:false,
    ...
    return withCors(
      json(
        ...
        {
          'Set-Cookie':
            `VIP_ADMIN_SESSION=${encodeURIComponent(token)}; Max-Age=${SESSION_TTL}; Path=/; HttpOnly; Secure; SameSite=Lax`
        }
      )
    );
  }
  ...
  if(path==='/api/device/register'&&request.method==='POST'){
    const body=await request.json().catch(()=>({})),
          deviceId=String(
            body.deviceId||
            request.headers.get('X-ViP-Device-ID')||
            ''
          );
    ...
    if(!d){
      d={
        deviceId,
        name:String(body.name||'Unknown device'),
        userAgent:String(
          body.userAgent||
  ...
  if(path==='/api/xtream/import'&&request.method==='POST'){
    const b=await request.json().catch(()=>({})),
          server=String(b.server||'').replace(/\/$/,''),
    ...
    if(!server||!user||!pass){
      return withCors(
        json(
          {
            ok:false,
            error:'server, username and password required'
          },
          400
        )
      );
    ...
    if(!Array.isArray(data)){
      return withCors(
        json(
          {
            ok:false,
            error:'Xtream returned invalid data'
          },
          400
        )
      );
    ...
    const lim=String(b.limit||'all'),
          items=lim==='all'
            ?data
            :data.slice(0,Number(lim)||100),
          base=
            server+
            '/live/'+
            encodeURIComponent(user)+
    ...
    const channels=items.map(x=>
      norm({
        name:x.name||('Channel '+x.stream_id),
        category:x.category_name||'Other',
        logo:x.stream_icon||'',
        url:
          base+
          encodeURIComponent(String(x.stream_id))+
          '.m3u8',
        status:'Unknown'
      })
    );
  ...
  if(path==='/api/admin/devices/approve'&&request.method==='POST'){
    const b=await request.json().catch(()=>({})),
          id=String(b.deviceId||''),
          list=await readDevices(env),
          d=list.find(x=>x.deviceId===id);
  ...
  if(path==='/api/admin/devices'&&request.method==='DELETE'){
    const id=url.searchParams.get('deviceId')||'',
          list=(await readDevices(env)).filter(
            x=>x.deviceId!==id
          );
