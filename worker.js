...
async function revokeUserSessionByDevice(e,device){
  if(!device)return;
  const users=await readUsers(e);
  let changed=false;
  for(const user of users){
    const match =
      String(user.activeDeviceId||'')===String(device.deviceId||'') ||
      String(user.lastDeviceId||'')===String(device.deviceId||'') ||
      (device.username && String(user.username||'').toLowerCase()===String(device.username||'').toLowerCase());
    if(match){
  ...
  if(changed) await saveUsers(e,users);
}
async function blockDeviceRecord(e,d,reason='admin'){
  if(!d)return;
  if(!d.blocked){
    d.previousApproved = !!d.approved;
    d.previousStatus = String(d.status||'');
    d.previousBlocked = false;
    d.blockedAt = new Date().toISOString();
    d.blockReason = reason;
  }
  d.blocked=true;
  d.approved=false;
  d.status='Blocked';
  await revokeUserSessionByDevice(e,d);
}
async function unblockDeviceRecord(d){
  if(!d)return;
  d.blocked=false;
  if(typeof d.previousApproved==='boolean') d.approved=d.previousApproved;
  else if(typeof d.approved!=='boolean') d.approved=false;
  if(d.previousStatus!==undefined) d.status=d.previousStatus || (d.approved?'Approved':'Logged out');
  else d.status=d.approved?'Approved':'Logged out';
  delete d.previousApproved;
  delete d.previousStatus;
  delete d.previousBlocked;
  delete d.blockedAt;
  delete d.blockReason;
}
...
async function handle(r,e){
  ...
  if(p==='/api/guest/register'&&r.method==='POST'){
    const b=await r.json().catch(()=>({}));
    const id=String(b.visitorId||'').trim().slice(0,160),deviceId=String(b.deviceId||'').trim().slice(0,160);
    ...
    if(!g){g={visitorId:id,deviceId,deviceName,userAgent:ua,ip,createdAt:now,lastSeen:now,status:'Online',blocked:false,category:'',channel:''};gs.push(g)}
  ...
  if(p==='/api/guest/ping'&&r.method==='POST'){
    const b=await r.json().catch(()=>({})),id=String(b.visitorId||'').trim().slice(0,160),deviceId=String(b.deviceId||'').trim().slice(0,160);
    if(!id)return withCors(json({ok:false,error:'visitorId required'},400));
    if(deviceId&&await deviceBlocked(e,deviceId))return withCors(json({ok:false,blocked:true,error:'এই ডিভাইসটি ব্লক করা হয়েছে'},403));
    ...
    if(!g){
      ...
      g.deviceId=deviceId||g.deviceId;g.status='Online';if(b.category!==undefined)g.category=String(b.category).slice(0,80);if(b.channel!==undefined)g.channel=String(b.channel).slice(0,200);
  ...
  if(p==='/api/device/register'&&r.method==='POST'){
    const b=await r.json().catch(()=>({})),id=String(b.deviceId||r.headers.get('X-ViP-Device-ID')||'').slice(0,160);
    ...
    if(d?.blocked)return withCors(json({ok:false,error:'Blocked',blocked:true,device:d},403));
    if(!d){d={deviceId:id,name:String(b.name||'Unknown device'),userAgent:String(b.userAgent||r.headers.get('user-agent')||'').slice(0,200),approved:false,blocked:false,createdAt:new Date().toISOString(),lastSeen:new Date().toISOString()};ds.push(d)}
    else {d.lastSeen=new Date().toISOString();if(b.name)d.name=String(b.name).slice(0,100);if(b.userAgent)d.userAgent=String(b.userAgent).slice(0,200);}
    await saveDevices(e,ds);return withCors(json({ok:true,device:d,settings:await readSettings(e)}))
  }
  ...
  for(const action of ['block','unblock','approve'])if(p==='/api/admin/devices/'+action&&r.method==='POST'){
    ...
    if(action==='block'){await blockDeviceRecord(e,d,'admin-device-block');}
    if(action==='unblock'){await unblockDeviceRecord(d);}
    if(action==='approve'){d.approved=true;d.blocked=false;d.status='Approved';delete d.previousApproved;delete d.previousStatus;delete d.blockedAt;delete d.blockReason;}
  ...
  if(p==='/api/admin/guests/block'&&r.method==='POST'){
    ...
    if(!g.blocked){g.previousStatus=String(g.status||'Offline');g.blockedAt=new Date().toISOString();}
    g.blocked=true;g.status='Blocked';
    const ds=await readDevices(e);
    for(const d of ds){
      if((g.deviceId&&String(d.deviceId)===String(g.deviceId)) || (d.visitorId&&String(d.visitorId)===String(g.visitorId))){
        await blockDeviceRecord(e,d,'admin-guest-block');
      }
    }
    await saveDevices(e,ds); await saveGuests(e,gs);
    return withCors(json({ok:true,guest:g}));
  }
  if(p==='/api/admin/guests/unblock'&&r.method==='POST'){
    const b=await r.json().catch(()=>({})),gs=await readGuests(e),g=gs.find(x=>x.visitorId===String(b.visitorId||''));
    if(!g)return withCors(json({ok:false,error:'Guest not found'},404));
    g.blocked=false;g.status=g.previousStatus||'Offline';delete g.previousStatus;delete g.blockedAt;
    const ds=await readDevices(e);
    for(const d of ds){
      if((g.deviceId&&String(d.deviceId)===String(g.deviceId)) || (d.visitorId&&String(d.visitorId)===String(g.visitorId))){
        await unblockDeviceRecord(d);
      }
    }
    await saveDevices(e,ds); await saveGuests(e,gs);
    return withCors(json({ok:true,guest:g}));
