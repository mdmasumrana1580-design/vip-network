...
async function handle(r, e) {
  ...
  if (p === '/api/device/register' && r.method === 'POST') {
    ...
    if (!d) {
      d = { deviceId: id, name: String(b.name || 'Unknown device'), userAgent: String(b.userAgent || r.headers.get('user-agent') || '').slice(0, 200), approved: false, blocked: false, createdAt: new Date().toISOString(), lastSeen: new Date().toISOString() };
      ds.push(d);
    } else d.lastSeen = new Date().toISOString();
    await saveDevices(e, ds);
    return withCors(json({ ok: true, device: d, settings: await readSettings(e) }));
  }
  ...
  if (p === '/api/admin/devices' && r.method === 'DELETE') {
    ...
    await saveDevices(e, (await readDevices(e)).filter(x => x.deviceId !== id));
    return withCors(json({ ok: true }));
  }
