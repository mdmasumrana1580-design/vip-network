  ...
  if (p === '/api/user/login' && r.method === 'POST') {
    ...
    if (!user) {
      ...
      user = {
        ...
        lastDeviceId: null, lastDeviceName: '', number: number || '', ip: ip, activeSessionToken: null
      };
      users.push(user);
      created = true;
    ...
    if (!created && user.activeSessionToken) {
      ...
      if (activeRaw) {
        ...
        if (active.deviceId && deviceId && active.deviceId !== deviceId) {
          return withCors(json({
            ok: false,
            error: 'এই অ্যাকাউন্ট অন্য একটি ডিভাইসে লগইন করা আছে। আগে Logout করুন।',
    ...
    user.ip = ip || user.ip || '—';
    user.lastLoginBD = bangladeshTime(user.lastLoginAt);
    const t = token();
    ...
    if (deviceId) {
      ...
      if (!d) {
        d = {
          deviceId, name: deviceName, deviceName, userName: user.username, username: user.username, number, ip, userId: user.id, userAgent: r.headers.get('user-agent') || '',
          approved: true, blocked: false, status: 'Logged in',
          createdAt: new Date().toISOString(), lastSeen: new Date().toISOString(), lastLoginAt: user.lastLoginAt, lastLoginBD: user.lastLoginBD
        };
        ds.push(d);
      } else {
        d.name = deviceName || d.name;
        d.deviceName = deviceName || d.deviceName || d.name;
        ...
        d.lastLoginAt = user.lastLoginAt;
        d.lastLoginBD = user.lastLoginBD;
        d.status = 'Logged in';
        d.blocked = false;
      }
    ...
    return withCors(json({ ok: true, created, username: user.username }, 200, {
      headers: {
        'Set-Cookie': `VIP_USER_SESSION=${encodeURIComponent(t)}; Max-Age=${USER_TTL}; Path=/; HttpOnly; Secure; SameSite=Lax`
      }
    }));
  }
