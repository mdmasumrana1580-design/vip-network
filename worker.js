const STATE_KEY='vip_state_v1',DEVICES_KEY='vip_devices_v1',SETTINGS_KEY='vip_settings_v1',USERS_KEY='vip_users_v1',ONLINE_KEY='vip_online_v1',ADMIN_SESSION_PREFIX='vip_admin_session:',USER_SESSION_PREFIX='vip_user_session:',ADMIN_TTL=1800;
const adminPassword=e=>e.ADMIN_PASSWORD||e.ADMIN_PASSWOED||'';const kv=e=>e.VIP_PLAYLIST||e.PLAYLIST_KV;const ONLINE_MEMORY=new Map();const cors={'access-control-allow-origin':'*','access-control-allow-methods':'GET,POST,PUT,DELETE,OPTIONS','access-control-allow-headers':'Content-Type, Authorization, X-ViP-Device-ID'};
...
async function readState(e){const r=await kv(e).get(STATE_KEY);try{return r?JSON.parse(r):{channels:[],notice:{text:'',type:'Information',enabled:true},headline:''}}catch{return{channels:[],notice:{},headline:''}}}async function saveState(e,s){await kv(e).put(STATE_KEY,JSON.stringify(s))}async function readDevices(e){try{return JSON.parse((await kv(e).get(DEVICES_KEY))||'[]')}catch{return[]}}async function saveDevices(e,x){const data=JSON.stringify(Array.isArray(x)?x:[]);let last;for(let i=0;i<2;i++){try{await kv(e).put(DEVICES_KEY,data);return true}catch(err){last=err}}throw last||new Error('Device data could not be saved')}async function readSettings(e){try{return JSON.parse((await kv(e).get(SETTINGS_KEY))||'{"deviceLimit":1,"accessMode":"approval"}')}catch{return{deviceLimit:1,accessMode:'approval'}}}async function readUsers(e){try{return JSON.parse((await kv(e).get(USERS_KEY))||'[]')}catch{return[]}}async function saveUsers(e,x){const data=JSON.stringify(Array.isArray(x)?x:[]);await kv(e).put(USERS_KEY,data);return true}function cleanOnlineMap(now=Date.now()){for(const[k,v]of ONLINE_MEMORY){if(now-v>=120000)ONLINE_MEMORY.delete(k)}}
...
async function adminCookie(r,e){const t=cookies(r).VIP_ADMIN_SESSION;if(!t)return false;const [ts,sig]=String(t).split('.');const n=Number(ts);if(!Number.isFinite(n)||Date.now()-n>ADMIN_TTL*1000)return false;const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(adminPassword(e)),{name:'HMAC',hash:'SHA-256'},false,['verify']);const data=new TextEncoder().encode(String(ts));const bytes=new Uint8Array((sig||'').match(/.{1,2}/g)?.map(h=>parseInt(h,16))||[]);return bytes.length===32&&await crypto.subtle.verify('HMAC',key,bytes,data)}
async function adminCookieToken(e){const ts=String(Date.now());const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(adminPassword(e)),{name:'HMAC',hash:'SHA-256'},false,['sign']);const b=new Uint8Array(await crypto.subtle.sign('HMAC',key,new TextEncoder().encode(ts)));const sig=[...b].map(x=>x.toString(16).padStart(2,'0')).join('');return ts+'.'+sig}
async function authorized(r,e){const p=adminPassword(e);if(!p)return false;if(await adminCookie(r,e))return true;return r.headers.get('Authorization')===`Bearer ${p}`}async function requireAdmin(r,e){return(await authorized(r,e))?null:json({ok:false,error:'Unauthorized'},401)}
...
const now=Date.now();cleanOnlineMap(now);ONLINE_MEMORY.set(id,now);
return withCors(json({ok:true,online:ONLINE_MEMORY.size}));
}
...
if(p==='/api/user/login'&&r.method==='POST'){
 ...
 const t=token();
 await kv(e).put(USER_SESSION_PREFIX+t,JSON.stringify({userId:user.id,username:user.username,deviceId,createdAt:new Date().toISOString()}));
 return withCors(json({ok:true,created,username:user.username},200,{headers:{'Set-Cookie':`VIP_USER_SESSION=${encodeURIComponent(t)}; Expires=Fri, 31 Dec 2099 23:59:59 GMT; Path=/; HttpOnly; Secure; SameSite=Lax`}}));
}
if(p==='/api/user/session'&&r.method==='GET'){const s=await userSession(r,e);if(!s)return withCors(json({ok:false,error:'Unauthorized'},401));const ds=await readDevices(e);const d=ds.find(x=>String(x?.deviceId||'')===String(s.deviceId||''));if(!d)return withCors(json({ok:false,error:'Device removed',removed:true},401));if(d.blocked)return withCors(json({ok:false,error:'Blocked',blocked:true},403));return withCors(json({ok:true,username:s.username,deviceId:s.deviceId,device:d}))}
...
if(p==='/api/admin/ping'&&r.method==='GET')return withCors(json({ok:true,connected:true}));if(p==='/api/admin/state'&&r.method==='GET')return withCors(json({ok:true,state:await readState(e)}));if(p==='/api/admin/state'&&r.method==='PUT'){const b=await r.json().catch(()=>({})),s={...(await readState(e)),...b};if(Array.isArray(b.channels))s.channels=b.channels.map(norm);await saveState(e,s);return withCors(json({ok:true,state:s}))}
if(p==='/api/admin/users'&&r.method==='GET'){
const users=await readUsers(e),ds=await readDevices(e);
const safe=users.map(u=>({
id:u.id,username:u.username,createdAt:u.createdAt||null,lastLoginAt:u.lastLoginAt||null,
