...
async function handle(r,e){
  ...
  if(p==='/api/user/login'&&r.method==='POST'){
    ...
    const now=new Date().toISOString();user.number=number;user.lastLoginAt=now;user.lastDeviceId=deviceId||null;user.lastDeviceName=deviceName;const t=token();user.activeSessionToken=t;user.activeDeviceId=deviceId||null;await saveUsers(e,users);
