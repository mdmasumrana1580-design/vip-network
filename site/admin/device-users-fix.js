  ...
  async function loadUsersFixed(){
    ...
    try{
      ...
      box.innerHTML=list.length?list.map(x=>{
        const blocked=x.status==='Blocked'||x.blocked===true, name=x.userName||x.username||x.user||'Guest', number=x.number||x.mobile||x.phone||'—', device=x.deviceName||x.name||'My Device', ip=x.ip||x.ipAddress||'—', when=x.lastLogin||x.lastSeen||x.createdAt||'';
