...
(function(){
  ...
  const bdTime=v=>{try{return new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Dhaka',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(new Date(v))+' BD'}catch{return v||'—'}};
  async function loadUsersFixed(){
    ...
    try{
      ...
      box.innerHTML=list.length?list.map(x=>{
        const blocked=x.status==='Blocked'||x.blocked===true, rawUser=x.userName||x.username||x.user||'', device=x.deviceName||x.name||'Mobile Device', name=(rawUser && String(rawUser).trim().toLowerCase()!=='guest')?rawUser:device, number=x.number||x.mobile||x.phone||'—', ip=x.ip||x.ipAddress||'—', when=x.lastLogin||x.lastSeen||x.createdAt||'';
    ...
    }catch(e){if(/Unauthorized/i.test(e.message)){connected=false;showLogin(true)}box.innerHTML='<div>Could not load users: '+esc2(e.message)+'</div>'}
  }
