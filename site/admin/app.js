...
let connected=false,channels=[],categories=['ALL','NEW STYLE','SPORTS','BD','INDIA','OTHER','MOVIE & SERIES'],selected=-1,deferredPrompt=null;
...
async function loadRemoteState(){const d=await api('/api/admin/state');const s=d.state||d;channels=Array.isArray(s.channels)?s.channels.map(norm):[];categories=[...new Set([...(Array.isArray(s.categories)?s.categories:['ALL','NEW STYLE','SPORTS','BD','INDIA','OTHER','MOVIE & SERIES']),...channels.map(x=>x.category).filter(Boolean)])];saveLocal();render();if($('syncTime'))$('syncTime').textContent=new Date().toLocaleString()}
...
async function removeCategory(i){const n=categories[i];if(['ALL','NEW STYLE','SPORTS','BD','INDIA','OTHER','MOVIE & SERIES'].includes(n))return toast('Default category cannot be deleted');if(channels.some(x=>x.category===n))return toast('Category is in use');categories.splice(i,1);if(await saveRemoteState()){render();toast('Category removed')}}
