/* VIP NETWORK ADMIN - safe category/New Style patch */
const DEFAULT_WORKER_API = (window.VIP_ADMIN_WORKER_API || 'https://vip-network-tv.vip-network.workers.dev');
let WORKER_API = DEFAULT_WORKER_API.replace(/\/$/, "");
let connected = false;
let channels = [];
const FIXED_NEW_STYLE = 'New Style';
let categories = ['New Style'];
...
async function loginWorker(){const url=document.getElementById('workerUrl')?.value.trim().replace(/\/$/,'');const pw=document.getElementById('workerPassword')?.value;if(!url||!pw)return setLoginStatus('Enter Worker URL and Admin Password.',true);WORKER_API=url;setLoginStatus('Connecting…');try{await api('/api/admin/login',{method:'POST',body:JSON.stringify({password:pw})});connected=true;setBackendState(true);showLogin(false);document.getElementById('workerPassword').value='';await loadRemoteState();await loadDevices();toast('Connected • session valid for 5 minutes');}catch(e){connected=false;setBackendState(false);setLoginStatus('Login failed: '+e.message,true);}}
...
function mergeCategories(existing, channelList){
  const arr = Array.isArray(existing)?existing:[];
  const fromChannels = Array.isArray(channelList)?channelList.map(c=>c.category).filter(Boolean):[];
  return [...new Set([FIXED_NEW_STYLE,...arr,...fromChannels])];
}
async function loadRemoteState(){
  if(!connected)return;
  try{
    const data=await api('/api/admin/state');const s=extractState(data);
    channels=s.channels;categories=mergeCategories(data?.state?.categories||data?.categories,channels);
  ...
  }catch(e){if(String(e.message).toLowerCase().includes('unauthorized')){connected=false;setBackendState(false);showLogin(true);setLoginStatus('Session expired. Please login again.',true);}toast('Could not load Worker state: '+e.message);}
}
async function saveRemoteState(extra={}){
  if(!connected){toast('Worker is not connected. Login first.');return false;}
  try{await api('/api/admin/state',{method:'PUT',body:JSON.stringify({channels,categories,...extra})});saveLocal();const sync=document.getElementById('syncTime');if(sync)sync.textContent=new Date().toLocaleString();return true;}
...
function render(){
  const total=channels.length,e=id=>document.getElementById(id);
  if(e('totalStat'))e('totalStat').textContent=total;if(e('activeStat'))e('activeStat').textContent=channels.filter(c=>c.status==='Active').length;if(e('deadStat'))e('deadStat').textContent=channels.filter(c=>c.status==='Dead').length;if(e('unknownStat'))e('unknownStat').textContent=channels.filter(c=>c.status!=='Active'&&c.status!=='Dead').length;
  ...
  if(e('categoryList')){
    let html='<span class="category-all">ALL</span>';
    html+=`<span class="category-new-style">NEW STYLE <button class="category-remove" onclick="removeCategoryByName('New Style')">×</button></span>`;
    categories.filter(c=>c!==FIXED_NEW_STYLE).forEach(c=>{html+=`<span class="category-normal">${esc(c)} <button class="category-remove" onclick="removeCategoryByName(${JSON.stringify(c).replace(/</g,'\\u003c')})">×</button></span>`});
    e('categoryList').innerHTML=html;
  }
}
...
async function addCategory(){const n=document.getElementById('newCat')?.value.trim();if(!n)return;if(n.toLowerCase()==='all')return toast('ALL is a filter only');if(categories.includes(n))return toast('Category already exists');categories.push(n);if(await saveRemoteState()){render();document.getElementById('newCat').value='';toast('Category added')}else{categories.pop();render()}}
async function removeCategoryByName(n){if(n===FIXED_NEW_STYLE)return toast('NEW STYLE cannot be removed');const i=categories.indexOf(n);if(i<0)return;if(channels.some(c=>c.category===n))return toast('Category is in use');const old=categories.splice(i,1)[0];if(await saveRemoteState()){render();toast('Category removed')}else{categories.splice(i,0,old);render()}}
async function removeCategory(i){return removeCategoryByName(categories[i])}
...
(async function init(){
  ...
  const cats=JSON.parse(localStorage.getItem('vipCategories')||'null');if(Array.isArray(cats))categories=mergeCategories(cats,channels);
  else categories=mergeCategories([],channels);
  render();
  const input=document.getElementById('workerUrl');if(input)input.value=DEFAULT_WORKER_API;
  try{await api('/api/admin/session');connected=true;showLogin(false);setBackendState(true);await loadRemoteState();await loadDevices()}catch{showLogin(true)}
})();
