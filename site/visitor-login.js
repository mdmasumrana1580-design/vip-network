(function(){
  ...
  function makeGate(){
    ...
    d.innerHTML=`<div class="vip-gate-card" role="dialog" aria-modal="true" aria-label="VIP-Network.TV Login">
      ...
      <form id="vipVisitorLogin" class="vip-login-form"><label class="vip-input"><span>👤</span><input id="vipUserName" autocomplete="name" placeholder="নাম" required></label><label class="vip-input"><span>📱</span><input id="vipUserNumber" inputmode="numeric" autocomplete="tel" placeholder="নাম্বার" maxlength="15" required></label><label class="vip-input"><span>💻</span><input id="vipDeviceName" autocomplete="off" placeholder="ডিভাইস নেম" required></label><button class="vip-login-btn" type="submit">⇥ &nbsp; LOGIN</button><div class="vip-gate-status" id="vipGateStatus"></div></form>
